...
class ChatToItem(val message: Message , val user : User): Item<ViewHolder>(){
    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
        viewHolder.itemView.tv_to_row.text = message.text
        val timePatterns = SimpleDateFormat("hh:mm")
        val postMessage = Date(message.timestamp)
        viewHolder.itemView.tv_time_to.text = timePatterns.format(postMessage)
        Picasso.get()
            .load(user.photoUrl)
            .into(viewHolder.itemView.iv_profile_chat2)
    }
}