		...
		<div class="panel panel-default articles">
			<div class="panel-heading">
				...
				<span class="pull-right panel-button-tab-right"> 
					...
					<Select class="form-control" id="filter-index"> 
						<option value="verify-1"> Uncategorized </option>
						<option value="verify-semua"> Tampilkan Semua </option>
						<option value="verify-2"> Disetujui </option>
						<option value="verify-3"> Ditolak </option>
					</Select>
				</span>
			...
			<table class="table table-striped" id="tabel-data">
				...
				<tbody>
					...
					<tr style="{{$item['verify'] > 1? 'display:none;':''}}" class="verify-{{$item['verify']}}">
						...
						<td> <h5><a class="report-data" id='{{ $item['id'] }}' href="#" data-toggle="modal" data-target="#exampleModalLong">{{ $item['title'] }}</a></h5></td>
						 ...
						 @if ($item['verify'] ==1 )
							...
							<div style="display: inline-block">
								<form action="/firebase/{{ $item['id'] }}" method="post">
									@csrf
									<input type="hidden" name="status" id="verify" value="2">
							...
							<div style="display: inline-block">
								<form  action="/firebase/{{ $item['id'] }}" method="post">
									@csrf
									<input type="hidden" name="status" id="verify" value="3">
							...
							Kasus disetujui oleh admin.

							@else 
							Kasus ditolak oleh admin.
